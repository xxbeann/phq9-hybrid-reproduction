# PHQ-9 하이브리드 문항 선택 기법의 전체 척도 재현도 검증 — 실험 재현 코드

학사학위논문 **「PHQ-9 기반 하이브리드 문항 선택 기법의 전체 척도 재현도 검증 — 정신건강
자가진단 서비스 「당신의 하루는」을 중심으로」**(전남대학교 컴퓨터정보통신공학전공)의 실험을
재현하는 코드와 산출물이다.

**핵심 질문**: PHQ-9 9문항 중 **고정 3문항(흥미 상실·우울 기분·수면) + 매 회 무작위 2문항**,
...
않는다.** KDCA(https://knhanes.kdca.go.kr)에서 직접 원시자료를 내려받아 `src/prep_knhanes.py`
로 동일한 처리 CSV를 재생성할 수 있다. 논문·`results/` 의 KNHANES **집계 결과**(평균·κ 등)는
