# PHQ-9 하이브리드 문항 선택 — 전체 척도 재현도 검증 (실험 재현)

학사학위논문 **「LLM(Qwen)을 적용한 PHQ-9 하이브리드 자가진단 기법의 효율적 구현: 정신건강
자가진단 서비스 「당신의 하루는」을 중심으로」** <br>(전남대학교 컴퓨터정보통신공학전공)의 **실험
파트(문항 축소에 따른 전체 척도 재현도)**를 재현하는 코드와 산출물이다. 일기에서 대규모
언어모델(Qwen/Gemma)이 문항을 판별하는 프로토타입은 동반 저장소
[`phq9-diary-prototype`](https://github.com/xxbeann/phq9-diary-prototype)에 있다.
